...
2. Place the downloaded JAR file in the `plugins`/`mods` (depends on your server software; `plugins` for Bukkit-based servers, `mods` for Forge and Fabric) folder of your Minecraft server.
...
If you have a Minecraft server from BerryByte, we've simplified all steps below to a few buttons on [Lime](../../lime/overview.md), where you can easily set up a reverse proxy for Dynmap using the [Proxies Manager](../../lime/proxies.md) tool. We take care of SSL certificates and all the configuration for you.
...
2. Head to the [Proxies Manager](../../lime/proxies.md) tool and click on the **Create Proxy** button.
...
5. Restart the web server to apply the changes: `sudo systemctl restart nginx`
